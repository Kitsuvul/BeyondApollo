...
public class ShipControlsScript : MonoBehaviour
{
    ...
    void Update()
    {
        baseShipScript.RocketAnimator.SetBool("HasLaunched", hasLaunched);
        baseShipScript.RocketAnimator.SetBool("IsBoosting", isBoosting);

        if (tutorialScript || !uiHandlerObj.GetComponent<TopLevelUIHandler>().UIIsOpen)
        {
        ...
        if (baseHelperScript.IsEndlessMode() && isBoosting && !this.GetComponent<BaseShipScript>().IsDead)
        {
            AddDirectionalForceInEndless();
        }
    }
    #endregion
    ...
    /// 
    /// </summary>
    private void AddDirectionalForceInEndless()
    {
        if (Input.mousePosition.x > (Screen.width / 2))
        {
            rocketShip.GetComponent<Rigidbody2D>().AddRelativeForce(Vector2.right * 10);
        }
        else
        {
            rocketShip.GetComponent<Rigidbody2D>().AddRelativeForce(Vector2.left * 10);
        }
    }
