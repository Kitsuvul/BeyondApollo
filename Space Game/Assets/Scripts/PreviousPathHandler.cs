...
public class PreviousPathHandler : MonoBehaviour
{
    ...
    private GameObject rocketShipObj, markerContainer, gameController;
    ...
    void Update()
    {
        if (controlsScript.HasLaunched && !rocketShipObj.GetComponent<BaseShipScript>().IsDead && helperScript.IsStoryMode()) // Add a pause for when you die
        {
            SpawnMarkers();
        }
    }
